  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-slate-200/70 bg-white/70 p-3 dark:border-slate-800 dark:bg-slate-900/60">
      ...
      <div className="min-w-0 flex-1">
        <Link to={profilePath(user)} className="block truncate text-sm font-bold hover:text-primary">
          {displayName(user, 'Member')}
        </Link>
        {!compact && <p className="truncate text-xs text-slate-500 dark:text-slate-400">{usernameHandle(user) || user.email || user.bio || 'Campus community member'}</p>}
        {compact && <p className="truncate text-xs text-slate-500 dark:text-slate-400">{usernameHandle(user) || user.bio || 'Suggested connection'}</p>}
      </div>
